...
fn get_configuration() -> MinecraftServerConfiguration {
	...
	if let Ok(configuration_string) = result {
		...
		println!("Unable to read the msc-configuration.ron file, falling back to default configuration.");
	}
...
fn get_names(config_option_info: Vec<ConfigurationOption>, configuration: MinecraftServerConfiguration) -> Vec<String> {
	let configuration_option_names: Vec<String> = config_option_info.into_iter()
		.map(|option_information| {
			let value: String = match configuration.get(option_information.property) {
				ConfigurationOptionType::Bool(value) => if value { "Enabled".to_string() } else { "Disabled".to_string() },
				ConfigurationOptionType::OptionString(value) => value.unwrap_or("default".to_string()),
...
fn run_server(configuration: MinecraftServerConfiguration, jar_filename: String, terminal: Term) -> (MinecraftServerConfiguration, Term) {
	...
	for option in get_config_option_info() {
		...
		let cli_flag = match (option.property.clone().as_str(), value) {
			("gui", ConfigurationOptionType::Bool(value)) => {
				if value { None } else { Some("--nogui".to_string()) }
			}
			(property, ConfigurationOptionType::Bool(value)) => {
				if value { Some(format!("--{}", property)) } else { None }
			}
			(property, ConfigurationOptionType::OptionU16(value)) => {
				match value {
					Some(value) => Some(format!("--{} {}", property, value.to_string())),
					None => None
				}
			}
			(property, ConfigurationOptionType::OptionString(value)) => {
				match value {
	...
	terminal.write_line(style("Starting your Minecraft Server").green().to_string().as_str()).unwrap();
	...
	match output {
		Ok(output) => {
			...
			if code == 0 {
				...
				terminal.write_line("Something went wrong! Do you have Java installed?").unwrap();
			}
		}
		Err(error) => {
			terminal.write_line(style("An error ocurred!").red().to_string().as_str()).unwrap();
...
fn main() {
	...
	let server_directory = matches.value_of("server_directory").unwrap_or(get_default_configuration_path().as_str()).to_string();
	set_current_dir(server_directory).expect("Expected to be able to change the current directory to your Minecraft Server's directory.");
	...
	while machine.state != AppState::Exited {
		terminal.clear_screen().expect("Expected to be able to clear the terminal.");

		match machine.state.clone() {
			AppState::ChoiceMenu => {
				terminal.set_title("Minecraft Server CLI — Choice Menu");
				let select_options = get_names(config_option_info.clone(), machine.configuration.clone());
				...
				if result == 0 {
					...
					match config_option_info.get(result - 2) {
						...
						None => {
							terminal.write_line("You didn't select one of the options. Please try again.").unwrap();
			...
			AppState::Running => {
				...
				let vars = run_server(machine.configuration.clone(), jar_filename.clone(), terminal.clone());
			...
			AppState::EditingConfiguration => {
				...
				let editor_state = machine.editor_state.clone().expect("Expected to have an editor state while in EditingConfiguration app state.");
				...
				match editor_state {
					EditorState::SelectOnOff => {
						...
						machine.dispatch(
							...
							Some(Payload::ConfigurationOptionType(ConfigurationOptionType::Bool(result == 0)))
						);
						...
						match result.parse::<u16>() {
							Ok(result) => {
								if result == 0 {
									...
									machine.dispatch(
										Event::EditorEvent(EditorEvent::SubmitValue),
										Some(Payload::ConfigurationOptionType(ConfigurationOptionType::OptionU16(Some(result))))
									);
									machine.configuration = save_configuration(machine.configuration);
								}
							}
					...
					EditorState::TextInput => {
						...
						machine.dispatch(
							Event::EditorEvent(EditorEvent::SubmitValue),
							Some(Payload::ConfigurationOptionType(ConfigurationOptionType::OptionString(Some(result))))
						);
						machine.configuration = save_configuration(machine.configuration);
					}
